defmodule MembraneTranscriptionTest do
  ...
  @filepath "test/samples/test_audio.mp3"
  ...
  defmodule Pipeline do
    ...
    def handle_init(opts) do
      ...
      children = %{
        ...
        transcription: %MyMembraneTranscription.Element{to_pid: to_pid, model: model},
  ...
  def receive_transcript(acc \\ []) do
    IO.puts("Waiting for transcript")
    ...
    receive do
      {:transcript, transcript} ->
        ...
        %{text: text} = hd(results)
        dbg(text)
        # dbg(.text)
        receive_transcript([text | acc])

      :transcription_done ->
        File.write!(
          "transcriptions/transcription_#{DateTime.utc_now() |> DateTime.to_unix()}.txt",
